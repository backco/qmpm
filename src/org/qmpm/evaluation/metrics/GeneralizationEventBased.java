...
public class GeneralizationEventBased extends Metric {
	...
	public Outcome doComputation(Trie t) throws LabelTypeException, NodeTypeException {
		...
		for (Node n : prefixNodeSet) {
			...
			if (!(n instanceof ModelNode)) throw new NodeTypeException("Can only calculate model-log metrics on tries consisting of ModelNodes. Encountered node of type: " + n.getClass().getSimpleName());
			...
			if (!n.getIsRoot()) {
				...
				generalization += pnew(diff.size(), simSize) * n.getVisits();
		...
		value = 1-(1.0/numOfEvents)*generalization;
